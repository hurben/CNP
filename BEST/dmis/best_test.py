...
class BESTQuery():
	...
	def getOidFromName(self):
		...
		while(again < 100000) :
			errormsg = "Network status is not good"
			try:
				...
				errormsg = "Client Excpetion"
				again += 1
			except socket.timeout:
				errormsg = "Network status is not good (time out)"
				again += 1
			except socket.error:
	...
	def makeQueryString(self, querytype):
		...
		if otype == "":
			return "Invalid type! type can be [gene, pathway, disease, drug,mirna]"
	...
	def makeQueryString_noAbsTxt(self, querytype):
		...
		if querytype == "gene":
			...
			otype = "10"
		if otype == "":
			return "Invalid type! type can be [gene, pathway, disease, drug]"
...
def getRelevantBioEntities(bestQuery):
	...
	* QUERY_OBJECT (dict): Instance of class BESTQuery
	...
	if not (type(bestQuery) is BESTQuery):
		...
		return {"queryResult" : "invalid query", "genes":[], "pathways":[], "diseases":[],"mirna":[]}
	...
	if bestQuery.noAbsTxt == True:
		...
		drugQuery = bestQuery.makeQueryString_noAbsTxt("drug") + fq
		mirnaQuery = bestQuery.makeQueryString_noAbsTxt("mirna") + fq
	else :
		...
		drugQuery = bestQuery.makeQueryString("drug") + fq
		mirnaQuery = bestQuery.makeQueryString("mirna") + fq
	
	
	geneResult = getQueryResult(geneQuery, fq, bestQuery)
	
	if type(geneResult) is int:
		return {"queryResult" : "invalid", "genes":[], "pathways":[], "diseases":[], "queryObject":bestQuery.toDataObj()}
	
	pathwayResult = getQueryResult(pathwayQuery, fq, bestQuery)
	
	if type(pathwayResult) is int:
		return {"queryResult" : "invalid", "genes":[], "pathways":[], "diseases":[], "queryObject":bestQuery.toDataObj()}
	
	diseaseResult = getQueryResult(diseaseQuery, fq, bestQuery)
	
	if type(diseaseResult) is int:
		return {"queryResult" : "invalid", "genes":[], "pathways":[], "diseases":[], "queryObject":bestQuery.toDataObj()}
	
	drugResult = getQueryResult(drugQuery, fq, bestQuery)
	
	if type(drugResult) is int:
		return {"queryResult" : "invalid", "genes":[], "pathways":[], "diseases":[], "queryObject":bestQuery.toDataObj()}
	
	mirnaResult = getQueryResult(mirnaQuery, fq, bestQuery)
	
	if type(mirnaResult) is int:
		return {"queryResult" : "invalid", "genes":[], "pathways":[], "diseases":[], "queryObject":bestQuery.toDataObj()}
	...
	return {"queryResult" : resultType, "genes":geneResult, "pathways":pathwayResult, "diseases":diseaseResult, "drugs":drugResult, "mirna":mirnaResult, "queryObject":bestQuery.toDataObj()}

def getQueryResult(queryStr, fq, bestQuery):
	...
	while(again < 100000) :
		errormsg = "Network status is not good"
		try:
			request = urllib.request.Request(queryStr)
			request.add_header('User-Agent', 'Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)')
			...
			errormsg = "Network status is not good: " + queryStr
			again += 1
		except http.client.HTTPException:
			errormsg = "Client Excpetion: " + queryStr
			again += 1
		except socket.timeout:
			errormsg = "Network status is not good (time out): " + queryStr
			again += 1
		except socket.error:
			errormsg = "Socket error: " + queryStr
			again += 1
		except urllib.error.URLError:
			errormsg = "URL is invalid: " + queryStr
			again += 1
		except Exception:
			errormsg = "Unknown exception: " + queryStr
			again += 1
...
def makeDataFromBestQueryResult_filtered(resultStr, filter):
	...
	for i in range(1, linesCnt) :
		...
		if line.startswith("@@@"):
			...
			curData = {"rank":int(dataResult[0].strip()), "entityName":dataResult[1].strip(), "score":float(dataResult[2].strip()), "numArticles":int(dataResult[3].strip()), "abstracts":str(dataResult[4].strip())}
	
	resultDataArr.append(curData)    
	
	for resultData in resultDataArr:
		try:
			if resultData["entityName"].lower() == filter.lower():
				return [resultData];
		except KeyError:
			continue
	return []
